...
protected int id;
     ...
     */
    public Item2(RefLinks refLink, float x, float y, int width, int height,int id)
    {
        this.id=id;
    ...
    public Rectangle getCollisionBounds2(float xOffset, float yOffset){
        return new Rectangle((int) (x + bounds.x + xOffset), (int) (y + bounds.y + yOffset), bounds.width, bounds.height);
    }

    public boolean checkEntityCollisions2(float xOffset, float yOffset){
        for(Item2 e : refLink.GetMap2().getItemManager2().getItems2()){
            if(e.equals(this))
                continue;
            if(e.getCollisionBounds2(0f, 0f).intersects(getCollisionBounds2(xOffset, yOffset)))
                return true;
        }
        return false;
    }
